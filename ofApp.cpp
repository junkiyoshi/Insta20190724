...
void ofApp::update() {
	...
	for (int i = 0; i < 8; i++) {
		...
		auto base_location = glm::vec3(ofRandom(-100, 100), ofRandom(-100, 100), ofRandom(-100, 100));
		...
		for (int deg = 0; deg < 360; deg += 5) {
			...
			this->locations.push_back(base_location + glm::vec4(point, 0) * rotation_x * rotation_y);
		}
	}
}
